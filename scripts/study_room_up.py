...
#
# 광역 최종 결과 (2026-09-22 · 유니버스 시총 상위 300 · 유효 23회차 · 비중첩 표본 22)
#   h=1 상위 3분위 초과 **-0.204%/회차** (양수 회차 9/22) · t검정 p 0.024.
#   하위 3분위 -0.296% → 스프레드 +0.092%p. 부호는 "위쪽 여유가 큰 쪽이 약간 낫다" 이다.
#
#   **t검정 p값은 비용을 재고 있다.** stat() 의 귀무가 '평균 0'인데 전략만 왕복 0.280%를
#   물어서, 신호가 전혀 없어도 -0.28% 가 나온다. 판정은 음성 대조군으로 한다 —
#     무작위 신호 200회 중앙 **-0.270%** · 관측 **-0.204%** → **신호 기여 +0.066%p**
#     단측 경험 p = **0.125** (무작위 200회 중 25회가 관측값 이상)
#
#   **결론 — 통과 못 함.** 두 가지 이유가 각각 독립적으로 결정적이다.
#     ① 유의하지 않다(단측 p 0.125).
#     ② 설령 점추정치를 그대로 믿어도 **신호 기여 0.066%p 는 왕복 비용 0.280%의 24%** 다.
#        이 항목만으로는 자기 거래비용도 못 벌고, 벤치마크 보유에 -0.204%/회차로 진다.
#
#   중간 결과가 표본에 따라 크게 흔들렸다 — 10회차에서는 대조군 대비 82%(관측이 더 나쁨)
#   였는데 22회차에서 12.5%(관측이 더 나음)로 뒤집혔다. **한 자릿수 회차로 방향을 말하면
#   안 된다**는 것을 이 검정 자체가 보여 준다. 좁은 검정(로스터 30종목)의 +0.152% 도 같은 부류다.
#
#   남은 격자: h=3·h=5 와 「규격 고정 후」(9/10~) 창은 **여전히 비중첩 표본 8 미만**이라
#   판정 불가다. 회차가 더 쌓여야 한다.
#
#   부수 — 2026-08-27 회차는 낡은 봉 216/298(72%)로 자동 제외됐다. 재수집해도 같았다.
#   그 날짜는 리플레이가 회차 도중 되돌아가는 문제가 재현된다.
#
# 사용법
