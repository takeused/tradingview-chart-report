...
def rows_of(html):
    """일봉 표를 종목별 구간으로 쪼갠다. 주봉 표와 순위 표는 제외한다.
...
def main():
    ...
    # 4-c) 필수 섹션이 살아 있는가 (2026-08-28 신설)
    #      서술을 손으로 갈아 끼우다가 끝 마커를 넘겨 잡아 「관전 포인트」와 「신규 편입」
    #      블록 둘이 통째로 지워진 채 발행됐다. 표만 대조하던 이 검사기는 못 잡았다 —
    #      **표가 맞아도 섹션이 사라질 수 있다.**
    #      2026-09-01에 「⭐ 베스트3」를 「🔁 순위」로 합쳤다(같은 종목이 두 블록에 두 번
